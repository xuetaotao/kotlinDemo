...
class FileUtils {
    ...
    companion object {
        ...
        fun compressImage(image: Bitmap): Bitmap? {
            ...
            while (baos.toByteArray().size / 1024 > 5000) {
         ...
         * @param picName
         */
        fun compressImageByQuality(bitmap: Bitmap, picName: String) {
            // 如果没有文件夹就创建一个程序文件夹
            if (!isFileExist(SDPATH)) {
                try {
                    val tempf = createSDDir(SDPATH)
                } catch (e: IOException) {
                    e.printStackTrace()
                }
            }
        ...
        fun deleteFile(file: File) {
            if (file.exists()) {// 判断文件是否存在
                if (file.isFile) {// 判断是否是文件
                    ...
                    val files: Array<File> = file.listFiles()// get声明目录下所有的文件 files[];
