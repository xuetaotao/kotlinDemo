...
object DensityUtil {
    ...
    fun isSmallWindow(activity: Activity): Boolean {
        ...
        //保证存的是屏幕宽度最大值
        if (metrics.heightPixels > MyMMKV.mmkv.decodeInt("max_size")) {
            MyMMKV.mmkv.encode("max_size", metrics.heightPixels)
        }
        //拿到的宽始终是一样的，高度也一样，但是小屏的高度比全屏小，所以这里首次存一下高度，后面用高度判断
        return metrics.heightPixels < MyMMKV.mmkv.decodeInt("max_size", metrics.heightPixels)
