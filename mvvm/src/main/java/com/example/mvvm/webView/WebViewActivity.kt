...
class WebViewActivity : BaseViewModelActivity<RoomViewModel>() {
    ...
    override fun initView() {
        ...
        toolbar_subtitle_image.setOnClickListener {
            if (!this@WebViewActivity::popWindow.isInitialized) {
                initPopWindow(it)
            } else if (popWindow.isShowing) {
                popWindow.dismiss()
            } else {
                popWindow.showAsDropDown(it, 0, 35)
            }
        }
        initWebView()
    }
    ...
    override fun providerVMClass(): Class<RoomViewModel> = RoomViewModel::class.java

    private fun addLater(link: String, title: String) {
        val mTitle = if (title != "") title else toolbar_title.text.toString()
        viewModel.addLater(link, title).observe(this) {
            toast("添加成功")
        }
    }
